...
class EvtFileOps:
    ...
    def readEF(self): # Reading fits event file from X-ray satellites
        ...
        evtFileKeyWords = {'TELESCOPE':TELESCOPE , 'INSTRUME':INSTRUME, 'OBS_ID':OBS_ID, 'TSTART':TSTART, 'TSTOP':TSTOP,
                               'LIVETIME':LIVETIME, 'TIMESYS':TIMESYS, 'MJDREF':MJDREF, 'TIMEZERO':TIMEZERO, 'DATEOBS':DATEOBS,
    ...
    def readGTI(self): # Reading fits event file GTI lists
        ...
        if TELESCOPE == 'XMM':
        ...
        elif TELESCOPE == 'NICER' or TELESCOPE == 'SWIFT' or TELESCOPE == 'NUSTAR' or TELESCOPE == 'IXPE':
            hdulist = fits.open(self.evtFile)
            ...
            gtiList = (np.vstack((ST_GTI, ET_GTI))).T
            if DATATYPE=="TTE":
                warnings.warn("Default GTI of GBM TTE file is simply start and end time of day. Run appendGTIsToTTE.py for a quick fix.", stacklevel=2)
    ...
    def filtEneEF(self, eneLow: float, eneHigh: float): # Filtering event file according to energy
        ...
        if TELESCOPE == 'NICER':
            ...
            warnings.warn("GBM only provides PHAs, and their conversion to energy is non-linear, hence, eneLow and eneHigh inputs are really PHA values. Use with care!", stacklevel=2)
            ...
            raise Exception('Check TELESCOP keyword in event file. Likely telescope not supported yet for energy filtering.')
    ...
    def addPhaseColEF(self, timMod: str, nonBaryEvtFile: str=""): # Filtering event file according to energy
        ...
        if nonBaryEvtFile:
            ...
            fits.update(evtFile, nonBaryNewhdulEFPH.data, nonBaryNewhdulEFPH.header, 'EVENTS')
...
if __name__ == '__main__':
    ...
    ################################################################################
    # Running the most common function - addPhaseColEF - for command line purposes #
    ################################################################################
    ...
    parser.add_argument("-nb","--nonBaryEvtFile", help="Name of non-barycentered event file",type=str, default="")

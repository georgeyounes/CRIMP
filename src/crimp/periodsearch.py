...
class PeriodSearch:
    ...
    def ztest(self):
        ...
        for jj in range(len(self.freq)):
            Z2pow[jj] = np.sum([((np.sum(np.cos(2 * (kk + 1) * np.pi * self.freq[jj] * (self.time-self.t0)))) ** 2 +
                                 (np.sum(np.sin(2 * (kk + 1) * np.pi * self.freq[jj] * (self.time-self.t0)))) ** 2) for kk in
    ...
    def twod_ztest(self, freq_dot):
        """
        Calculate 2-d Z^2-test power (at midpoint of TIME array)
        ...
        for ll in range(0, len(freq_dot)):
            for jj in range(len(self.freq)):
                ...
                z2nHarm = np.sum([(np.sum(np.cos(2 * (kk + 1) * np.pi *
                                                 (self.freq[jj] * (self.time-self.t0) +
                                                  (0.5 * (-1 * 10 ** freq_dot[ll]) * (self.time-self.t0) ** 2)))) ** 2) +
                                  (np.sum(np.sin(2 * (kk + 1) * np.pi * (self.freq[jj] * (self.time-self.t0) +
                                                                         (0.5 * (-1 * 10 ** freq_dot[ll]) *
                                                                          (self.time-self.t0) ** 2)))) ** 2)

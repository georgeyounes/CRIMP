...
# e.g., from crtPPTemplateMod.py), and a .txt file defining the ToAs (e.g., ToAs
...
# 3- tempModPP : *.txt file of the tempolate pulse profile (could be biult with crtPPTemplateMod.py)
...
from templateModels import fourSeries, logLikelihoodFSNormalized, wrapCauchy, vonmises
...
def measToAs(evtFile, timMod, tempModPP, toagtifile, eneLow=0.5, eneHigh=10., toaStart=0, toaEnd=None, phShiftRes=1000, nbrBins=15, varyAmps=False, plotPPs=False, plotLLs=False, toaFile='ToAs', timFile=None):
    ...
    if evtFileKeyWords["TIMESYS"] != "TDB":
        warnings.warn("Event file is not barycentered. This script is meant for ToA calculation where all TIMEs should be in TDB. Use with care!", stacklevel=2)
    ...
    f.write('ToA \t ToA_mid \t ToA_start \t ToA_end \t ToA_lenInt \t ToA_exp \t nbr_events \t count_rate \t phShift \t phShift_LL \t phShift_UL \t Hpower \t redChi2\n')
    ...
    for ii in range(toaStart,toaEnd):
        ...
        # Assigning 1 count to each phase
        #################################
        counts = np.ones(np.size(cycleFoldedPhases), dtype=int)

        # Measuring the best fit phase shift through an unbinned extended maximum likelihood fit
        ########################################################################################
        BFtempModPP = readPPTempAny(tempModPP) # BFtempModPP is a dictionary of parameters of best-fit template model

        if BFtempModPP["model"]==str.casefold('fourier'):
            ToAPropFourier = measToA_fourier(BFtempModPP, cycleFoldedPhases, ToA_exposure[ii], outFile=ToA_ID, phShiftRes=phShiftRes, nbrBins=nbrBins, varyAmps=varyAmps, plotPPs=plotPPs, plotLLs=plotLLs)
        ...
        ##############################
        ToAProp = {'ToA_mid': ToA_mid, 'phShi': ToAPropFourier["phShi"], 'phShi_LL': ToAPropFourier["phShi_LL"], 'phShi_UL': ToAPropFourier["phShi_UL"], 'htestPow': htestPowAtTmid, 'reducedChi2': ToAPropFourier["reducedChi2"]}
        
        # writing results to text file
        ##############################
        f.write(str(ii) + '\t' + str(ToAProp['ToA_mid']) + '\t' + str(ToAStartMJD[ii]) + '\t' + str(ToAEndMJD[ii]) + '\t' + str(ToA_lenInt[ii]) + '\t' + str(ToA_exposure[ii]) + '\t' + str(Events[ii]) + '\t' + str(ct_rate[ii]) + '\t' + str(ToAProp['phShi']) + '\t' + str(ToAProp['phShi_LL']) + '\t' + str(ToAProp['phShi_UL']) + '\t' + str(ToAProp['htestPow']) + '\t' + str(ToAProp['reducedChi2']) + '\n')
...
def measToA_fourier(BFtempModPP, cycleFoldedPhases, exposureInt, outFile='', phShiftRes=1000, nbrBins=15, varyAmps=False, plotPPs=False, plotLLs=False):
    
    initTempModPPparam = Parameters() # Initializing an instance of Parameters based on the best-fit template model
    initTempModPPparam.add('norm', BFtempModPP['norm'], min=0.0, max=np.inf, vary=True) # Adding the normalization - this is free to vary
    ...
    results_mle_FSNormalized = minimize(nll, initTempModPPparam, args=(cycleFoldedPhases, exposureInt), method='nelder', max_nfev=1.0e3, nan_policy='propagate')
    ...
    if varyAmps is True:
        ...
        initTempModPPparam.add('phShift', results_mle_FSNormalized.params.valuesdict()['phShift'], vary=True) # Phase shift - set to best fit value from above
        initTempModPPparam.add('norm', results_mle_FSNormalized.params.valuesdict()['norm'], min=0.0, max=1000., vary=True) # normalization - set to best fit value from above
        results_mle_FSNormalized = minimize(nll, initTempModPPparam, args=(cycleFoldedPhases, exposureInt), method='nelder', max_nfev=1.0e4, nan_policy='propagate')
    ...
    while chi2diff1sig <= chi2_1sig1dof:
        ...
        results_mle_forErrCalc = minimize(nll, initParam_forErrCalc, args=(cycleFoldedPhases, exposureInt), method='nedler', max_nfev=1.0e4, nan_policy='propagate')
    ...
    redchi2_pp = np.divide(chi2_pp,np.size(ppBins)-2) # We varied 2 parameters, the normalization and the phase-shift
...
def plotPhaseResiduals(ToAsMJD, phaseShifts, phaseShiftsLL, phaseShiftsUL, outFile):
    ...
    plt.errorbar(ToAsMJD, phaseShifts/(2*np.pi), yerr=(phaseShiftsLL/(2*np.pi), phaseShiftsUL/(2*np.pi)), fmt='ok',zorder=10)
...
if __name__ == '__main__':
    ...
    parser.add_argument("tempModPP", help="Parameters of template pulse profile (e.g., built with crtPPTemplateMod.py)", type=str)
    parser.add_argument("toagtifile", help="User supplied .txt file with ToA interval information (built with crtTimeIntForToAs.py)", type=str)
    ...
    parser.add_argument("-te","--toaEnd", help="Number of ToA to end, default full length of ToA list", type=int, default=None)
    parser.add_argument("-pr","--phShiftRes", help="Phase-shift step resolution, 2*pi/phShiftRes, default = 1000", type=int, default=1000)
    parser.add_argument("-nb","--nbrBins", help="Number of bins in PP for visualization purposes only, default = 15",type=int,default=15)
    parser.add_argument("-va","--varyAmps", help="Flag to allow the pulsed fraction of the template pulse profile to vary (not the shape!), default = False", type=bool, default=False, action=argparse.BooleanOptionalAction)
    parser.add_argument("-pp","--plotPPs", help="Flag to create pulse profile plots, default = False", type=bool, default=False, action=argparse.BooleanOptionalAction)
    parser.add_argument("-ll","--plotLLs", help="Flag to create LogLikelihood plots, default = False", type=bool, default=False, action=argparse.BooleanOptionalAction)
    parser.add_argument("-tf","--toaFile", help="name of output ToA file (default = ToAs(.txt))", type=str, default='ToAs')
    parser.add_argument("-mf","--timFile", help="name of output .tim file compatible with tempo2/PINT (default = None - no .tim file created)", type=str, default=None)
    args = parser.parse_args()
    
    measToAs(args.evtFile, args.timMod, args.tempModPP, args.toagtifile, args.eneLow, args.eneHigh, args.toaStart, args.toaEnd, args.phShiftRes, args.nbrBins, args.varyAmps, args.plotPPs, args.plotLLs, args.toaFile, args.timFile)

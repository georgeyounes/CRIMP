...
def timeIntForToAs(evtFile, totCtsEachToA=1000, waitTimeCutoff=1.0, eneLow=0.5, eneHigh=10):
    ...
    for ll in range(0, nbrGTIs):
        f.write(str(ToAs_timeInfo[ll][0]) + '\t' + str(ToAs_timeInfo[ll][1]) + '\t' + str(ToAs_timeInfo[ll][2]) + '\t' + str(ToAs_timeInfo[ll][3]) + '\n')
    ...
    for kk in range(0, nbrGTIs):
        ...
        if nbrToAs==1: # In case 1 ToA is possible within the desired WT interval
            ...
            f.write(str('{}'.format(nbrToATOT)) + '\t' + str('{:0.9f}'.format(timeToA[0]/86400+MJDREF)) + '\t' + str('{:0.9f}'.format(timeToA[-1]/86400+MJDREF)) + '\t' + str('{:0.9f}'.format(timeToA[-1]-timeToA[0])) + '\t' + str('{:0.9f}'.format(expToA_final)) + '\t' + str(len(timeToA)) + '\t' + str('{:0.9f}'.format(len(timeToA)/expToA_final)) + '\n')
            nbrToATOT += 1
            continue
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description="Creating time intervals for individual ToAs - saving info to .txt file")
    parser.add_argument("evtFile", help="Fits event file",type=str)
    parser.add_argument("-tc","--totCtsEachToA", help="Desired number of counts for ToA calculation",type=int, default=1000)
    parser.add_argument("-wt","--waitTimeCutoff", help="Do not allow any gap in ToA_GTI larger than this (in days)",type=float, default=1)
    parser.add_argument("-el","--eneLow", help="Low energy range for burst search (in keV units, default=0.5)",type=float,default=0.5)
    parser.add_argument("-eh","--eneHigh", help="High energy range for burst search (in keV units, default=10)",type=float,default=10)

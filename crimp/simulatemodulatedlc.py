...
# 3- amp : amplitude of sine function (in cts/s)
#
############################################################
...
def simulatemodulatedlc(freq, srcrate=1, exposure=10000, pulsedfraction=0.2, bgrrate=0.05, resolution=0.073, nbrPhaseBins=None):
    ...
    sim_cts = (srcrate+(amp*np.cos(2*np.pi*sim_p+np.pi))) * (exposure/nbrPhaseBins) # Peak is in the middle of a cycle
    ...
    for k in range (0,nbrPhaseBins):
        ...
        assigned_phase = nbrRot.astype(int) + np.random.uniform(phaseBin,phaseBin+(1/nbrPhaseBins),int(sim_cts[k]))
    ...
    assignedBgr_t_tmp = np.cumsum(np.random.exponential(1/bgrrate,totBackCts)) # Simulate waiting times for a poisson process
...
if __name__ == '__main__':
    ...
    parser.add_argument("-ex", "--exposure", metavar='', help="Exposure in seconds, default=10000.", type=float, default=10000.)
    parser.add_argument("-pf", "--pulsedfraction", metavar='', help="RMS pulsed fraction of signal, default=0.2.", type=float, default=0.2)
    parser.add_argument("-bg","--bgrrate", metavar='', type=float,default=0.05, help="background count rate, default=0.05")
    parser.add_argument("-rs","--resolution", metavar='', help="Resolution of instrument in seconds", type=float, default=0.073)
    parser.add_argument("-nb","--nbrPhaseBins", metavar='', help="Number of phase bins to draw source counts from", type=int, default=None)
    args = parser.parse_args()

    simulatemodulatedlc(args.freq,args.srcrate,args.exposure,args.pulsedfraction,args.bgrrate,args.resolution,args.nbrPhaseBins)
